...
class Statusfeed extends React.Component {
    ...
    GetPosts = async () => {
        ...
        const res = await axios.get(`${process.env.REACT_APP_APIURI}/posts/${this.state.idpost}?_where[user.username]=${this.state.user}`, {
    ...
    GetComments = async () => {
        ...
        const res = await axios.get(`http://localhost:1337/comments?_where[posts.id]=${this.state.idpost}&_sort=created_at:desc`, {
            cancelToken: source.token,
            headers: {
                Authorization: "Bearer " + localStorage.getItem("token"),
            }
    ...
    MGComm = async (e, comment) => {
        ...
        await axios.put("http://localhost:1337/comments/" + comment.id, {
            mg: comment.mg + 1,
